...
def get_course_info(course_slug):
    ...
    course_info["title"] = soup.find("h1", class_="title display-3-text").text
    course_info["language"] = soup.find("div", class_="language-info").text
    if not soup.find("div", class_="startdate") is None:
        course_info["start_date"] = soup.find("div", class_="startdate").text
    else:
        course_info["start_date"] = default_info
    if not soup.find_all("div", class_="week") is None:
        course_info["num_of_weeks"] = len(soup.find_all("div", class_="week"))
    else:
        course_info["num_of_weeks"] = default_info
    if not soup.find("div", class_="ratings-text") is None:
        course_info["rating"] = soup.find("div", class_="ratings-text").text
    else:
...
def output_courses_info_to_xlsx(filepath, courses):
    ...
    work_sheet.append(["Title", "Language", "Start date", "Length(Weeks)", "Rating"])
    for course in courses:
        work_sheet.append([course["title"], course["language"], course["start_date"], course["num_of_weeks"], course["rating"]])
    work_book.save("{}.xlsx".format(filepath)) 


if __name__ == '__main__':
    #parser = argparse.ArgumentParser()
    #parser.add_argument("filepath", help="Имя файла")
